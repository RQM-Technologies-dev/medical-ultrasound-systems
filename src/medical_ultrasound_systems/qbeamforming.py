...
def _quaternionic_from_signed_scalar_channels(
    channel_values: np.ndarray,
    axes: np.ndarray,
) -> np.ndarray:
    amplitude = np.abs(channel_values)
    phase = np.where(channel_values >= 0.0, 0.0, np.pi)
...
def quaternionic_delay_align_pixel(
    ...
    """Return delay-aligned quaternion values for one pixel.

    This is a first pixel-local quaternionic alignment prototype for synthetic
    research workflows and is not a calibrated scanner beamforming chain.
    """
    ...
    delayed_channels = sample_rf_nearest(rf, travel_times_s)
